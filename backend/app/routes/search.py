...
async def search_articles(text: str=None, tags: Annotated[list[str] | None, Query()]=None, limit: int=5, db: Session=Depends(get_db), es: Elasticsearch=Depends(get_elastic_search)):
    """
    ...
    try:
        results = ArticleCrud.search_articles(db, es, text=text, tags=tags, limit=limit)
